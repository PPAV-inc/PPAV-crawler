...
var _parse_film_info = require('./parse_film_info');

var _parse_film_info2 = _interopRequireDefault(_parse_film_info);
...
var receivedMessage = function receivedMessage(event) {
  ...
  switch (firstStr) {
    case 'PPAV':
      (0, _mongodb.findThreeVideos)(function (retrunArr) {
        retrunArr.forEach(function (value) {
      ...
      break;
    case '#':
      (0, _firebase.receiveVideoDataByCode)(messageText.split(' ')[1], function (returnObj) {
        var str = '';
        if (returnObj) {
          str = '片名：' + returnObj.title + '\n' + '番號：' + returnObj.code + '\n' + '女優：' + returnObj.models + '\n' + returnObj.url;
          sendTextMessage(senderID, str);
        } else {
          str = '搜尋不到此番號';
          sendTextMessage(senderID, str);
