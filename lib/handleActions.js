...
var receivedMessage = exports.receivedMessage = function receivedMessage(event) {
  ...
  if (messageText === 'PPAV' || messageText === 'ppav' || messageText === 'Ppav') {
    ...
    switch (firstStr) {
      case '＃':
      case '#':
        (0, _mongodb.findVideo)('code', messageText.split(firstStr)[1], function (returnArr) {
      ...
      case '％':
      case '%':
        (0, _mongodb.findVideo)('models', messageText.split(firstStr)[1], function (returnArr) {
          var str = '';
          if (returnArr.length == 0) {
            str = '搜尋不到此女優';
      ...
      case '＠':
      case '@':
        (0, _mongodb.findVideo)('title', messageText.split(firstStr)[1], function (returnArr) {
          var str = '';
          if (returnArr.length == 0) {
            str = '搜尋不到此片名';
            sendTextMessage(senderID, str);
