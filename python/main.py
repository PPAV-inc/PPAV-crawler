...
def update_url_info(url, old_collect, update_collect):
    date_info = old_collect.find_one({'url': url, 'update_date': {'$exists':True}}, \
                                {'update_date':1, '_id':0})
    ...
    if date_info is not None \
        ...
        info_is_exists = bool(old_collect.find_one({'url': url, 'title': {'$exists': True}})) 
        info = XONLINE.get_film_info(url, info_is_exists)

        print(info)
        if info_is_exists and info:
            old_collect.update_one({'url': info['url']}, {'$set': info}, upsert=True)
        if info:
            update_collect.update_one({'url': info['url']}, {'$set': info}, upsert=True)
...
if __name__ == '__main__':
    ...
    old_url_set = set(each['url'] for each \
                 in old_collect.find({'update_date': {'$exists':True}}, {'url':1, '_id':0}))
    update_url_set = set(each['url'] for each \
                 in update_collect.find({'update_date': {'$exists':True}}, {'url':1, '_id':0}))
    ...
    for idx, json in enumerate(info_list):
        if idx % 100 == 0 and idx > 0:
            print("update into collect {} : {} / {}".format('videos_new', idx, len(info_list)))
        old_collect.update_one({'url': json['url']}, {'$set': json}, upsert=True)
        new_collect.update_one({'url': json['url']}, {'$set': json}, upsert=True)
